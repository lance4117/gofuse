...
// PebbleConfig 是用于配置 Pebble 数据库的结构体。
// 它包含了数据库目录名称以及底层 pebble 的 Options 配置。
...
// 参数 dirname 表示数据库文件存放的目录路径。
// 返回值是一个初始化完成并设置了默认选项的 PebbleConfig 结构体。
...
// 参数 config 包含数据库目录名及配置选项。
// 若打开失败则记录致命错误日志，并返回 nil。
// 成功时返回封装好的 PebbleKV 指针。
...
// Put 向数据库中插入或更新指定 key 对应的 value 值。
// 参数 key 为字符串类型的键；val 为对应的字节切片值。
// 使用 NoSync 策略进行写入以提高性能。
// 返回可能发生的错误信息。
...
// 参数 key 为要查询的键。
// 如果找到对应值，则复制一份后返回该值及其 nil 错误；
// 如果未找到，则返回 ErrKeyNotFound 错误；
// 其他异常情况直接返回原始错误。
...
// 返回封装后的 Batch 接口实现和潜在错误（当前始终为 nil）。
...
// 参数 opt 控制迭代范围，如 Prefix 字段可用于限定前缀扫描区间。
// 返回封装好的 Iterator 接口实现及可能发生的错误。
